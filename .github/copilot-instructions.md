...
- `src/Dashboard.jsx` is the composition root for the display. It wires hard-coded Home Assistant entity IDs into the main weather/pool/hot tub layout, then delegates secondary panels to `components/Levels.jsx` and `components/Wind.jsx`.
...
- Vite accepts both `VITE_HASS_*` and legacy `REACT_APP_HASS_*` env names during the migration. Prefer the `VITE_` names for new setup or documentation.
...
- Pool season logic lives in `src/lib/poolSeason.js` so date-sensitive behavior can be tested without mocking the entire dashboard module.
